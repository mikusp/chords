...
void cq_dump_matrix_abs(const float _Complex* matrix, int height, int width) {
...
void cq_zero_vector_below_thresh(float _Complex* vec, int length,
        double thresh) {
    fftwf_complex zero = 0.0f + 0.0f*_Complex_I;
    for (int j = 0; j < length; ++j) {
        if (cabsf(vec[j]) <= thresh)
            vec[j] = zero;
    }
}
...
cs_ci* cq_make_kernel(int min_freq, int max_freq, int sample_rate,
    ...
    for (int j = K-1; j >= 0; --j) {
        ...
        //cq_zero_vector_below_thresh(spec, fft, 0.0054f);
